...
namespace ShapeGame
{
    ...
    public partial class MainWindow : Window
    {
        ...
        private void WindowClosed(object sender, EventArgs e)
        {
            ...
            Sessao mySessao = new Sessao();
            mySessao.closeSessao(labelUser.Text);
        }
        ...
        private async void Button_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (alive != 0)
            {
                //Console.WriteLine("Comecei a tentativa");
                FlyingText.NewFlyingText(this.screenRect.Width / 30, new Point(this.screenRect.Width / 2, this.screenRect.Height / 2), "Iniciando Tentativa!");
                for (int i = 3; i >= 0; i--)
                {
                    await Task.Delay(1000);
                    if (i != 0)
                        FlyingText.NewFlyingText(this.screenRect.Width / 30, new Point(this.screenRect.Width / 2, this.screenRect.Height / 2), i.ToString());
                }
                this.myFallingThings.NovaTentativa();
                FlyingText.NewFlyingText(this.screenRect.Width / 30, new Point(this.screenRect.Width / 2, this.screenRect.Height / 2), "VAI!");
                Sessao mySessao = new Sessao();
                mySessao.initTentativa(mySessao.buscaNome(labelUser.Text));
                btnInit.IsEnabled = false;
                btnClose.IsEnabled = true;
            }else{
                FlyingText.NewFlyingText(this.screenRect.Width / 40, new Point(this.screenRect.Width / 2, this.screenRect.Height / 2), "Usuário Não Encontrado");
            }
        }
        ...
        private void Button_Click1(object sender, RoutedEventArgs e)
        {
            ...
            if (alive != 0)
            {
                Console.WriteLine("Finalizei a Tentativa");
                foreach (var player in this.players)
                {
                    Sessao mySessao = new Sessao();
                    mySessao.closeTentativa(this.myFallingThings.PegaPonto(player.Value.GetId()));
                    dtGrid.DataContext = mySessao.newQuadro(mySessao.buscaNome(labelUser.Text));
                    Console.WriteLine(this.myFallingThings.PegaPonto(player.Value.GetId()));
